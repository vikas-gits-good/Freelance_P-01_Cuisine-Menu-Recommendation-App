...
class CypherFunctionTool:
    def __init__(self, cp_config: CypherCodeConfig = CypherCodeConfig()):
        try:
            load_dotenv()
            self.graph = GraphPool.get_graph(
                graph_name=os.getenv("KNOWL_GRAPH", "prod")  # type:ignore
            )
    ...
    def get_competitors_data(
        ...
        ## Usage:
        ```python
        >>> from Src.Config import CypherFunctionTool
    ...
    def get_competitors_menu(
        ...
        ## Usage:
        ```python
        >>> from from Src.Config import CypherFunctionTool
        >>> cft = CypherFunctionTool()
        >>> func_params = {
                "q_params": {
                    "area_ids": "area_Indiranagar__city_Bangalore-relation:7902476",
    ...
    def _process_data(
        ...
        try:
            ...
            result = self.graph.query(q_code, q_params, timeout=5000)
            df = pd.DataFrame(
                data=result.result_set,
                columns=[item[-1] for item in result.header],  # type: ignore
            )
            df = df.map(
                lambda row: (
